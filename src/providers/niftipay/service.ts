...
import {
  NiftipaySessionStore,
  type NiftipaySessionRow,
} from "./session-store"
...
class NiftipayPaymentProviderService extends AbstractPaymentProvider<NiftipayProviderOptions> {
  ...
  async initiatePayment(
    ...
    const brandSlug = optionalString(data.brand_slug)
    ...
    const description = renderTemplate(
      brandSettings?.descriptionTemplate ??
        this.options_.descriptionTemplate,
  ...
  async getPaymentStatus(
    ...
    const mapped: PaymentSessionStatus =
      status === "paid"
        ? "captured"
        : ["cancelled", "canceled", "expired", "refunded", "chargeback"].includes(
              status ?? "",
            )
          ? "canceled"
          : "pending"
  ...
  async getWebhookActionAndData(
    ...
    try {
      ...
      const session = await this.resolveSession(
        webhook.order.orderKey,
        webhook.order.merchantReference ?? webhook.order.reference,
      )
      if (!session || session.deleted_at || session.status === "canceled") {
        this.logger_.warn(
          `[niftipay] ${webhook.event} webhook has no live payment session (orderId=${webhook.order.id ?? "unknown"})`,
        )
        return unsupported
      }
